function [y,yh,ys] = apply_effects(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timemapping,fscale,timbremapping,bn)

% Analysis, transformation, and synthesis of a voice signal
% x: input sound
% fs: sampling rate
...
% minf0: minimum f0 frequency in Hz in Yin algorithm
% maxf0: maximim f0 frequency in Hz in Yin algorithm
% f0et: error threshold in the f0 detection
% maxhd: max. relative deviation in harmonic detection
% stocf: decimation factor of magnitude spectrum for stochastic analysis
...
while pout<poutend
    ...
    [ploc,pmag,pphase] = interp_peaks(mX,pX,ploc); % refine peak values
    ...
    if (yb<1 || ye>length(x)) % out of boundaries
        ...
        f0 = yindetectf0(x(yb:ye),fs,yinws,minf0,maxf0); % compute f0
    end
    ...
    Xh = sinestransform(hloc(1:hi-1),hmag,hphase,Ns, bn); % generate sines
    ...
    lastyhloc = yhloc;
    Yh = sinestransform(yhloc,yhmag,yhphase,Ns,bn); % generate sines
